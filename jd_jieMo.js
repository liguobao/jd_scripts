...
[task_local]
#芥么签到
46 0,9 * * * https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_jieMo.js, tag=芥么签到, enabled=true

================Loon==============
[Script]
cron "46 0,9 * * *" script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_jieMo.js, tag=芥么签到
          ...
          if (data) {
            ...
            if (data.success) {
              if (data.data.status === "310") {
				...
				//NodeMessage += `提现：${data.data.message}\n`;
              } else if (data.data.status === "1000") {
                console.log("今天已经完成提现了");
				//NodeMessage += `提现：今天已经完成提现了\n`;
              }
            } else {
          ...
          if (data) {
            ...
            if (data.success) {
              if (data.data.historySignInAnCycle) {
				...
				NodeMessage += `京东账号${$.index} ${$.nickName || $.UserName}\n签到：获得${data.data.historySignInAnCycle[0].prizeAwardVale}\n`;
              } else {
                console.log(data.data.retMessage)
				NodeMessage += `京东账号${$.index} ${$.nickName || $.UserName}\n签到：${data.data.retMessage}\n`;
              }
            } else {
              console.log(JSON.stringify(data));
            }
...
async function duty() {
	...
	NodeMessage += `未注册！无法做任务，请手动进入一次小程序任务\n\n`;
	} return;
}
