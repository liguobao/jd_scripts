...
[task_local]
#签到领现金
33 8-23/4 * * * jd_cash_new.js, tag=签到领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "33 8-23/4 * * *" script-path=jd_cash_new.js,tag=签到领现金

===============Surge=================
签到领现金 = type=cron,cronexp="33 8-23/4 * * *",wake-system=1,timeout=3600,script-path=jd_cash_new.js

============小火箭=========
签到领现金 = type=cron,script-path=jd_cash_new.js, cronexpr="33 8-23/4 * * *", timeout=3600, enable=true
 */
const $ = new Env('签到领现金');
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://share.jdym.cc/api/v1/jd/jdcash/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function getAuthorShareCode(url = "http://cdn.annnibb.me/jd_cash_new.json") {
  return new Promise(resolve => {
    $.get({url, headers:{
